...
/* TESTEANDO COM VERIFICAÇÃO DE ESTOQUE
bool Estoque::InserirUnd(CervejaUnd c_u)
{
    int total;
    // Atualizar o estoque
	for(unsigned i(0); i < CervejaUnidade.size(); i++)
	{
		// Verificando se já não existe uma cerveja na lista com o mesmo nome.
		if (CervejaUnidade[i].getNome() == c_u.getNome())
        {
            // inserir um nova cerveja no lote
            if(CervejaUnidade[i].getCodigo() != c_u.getCodigo())
            {
                CervejaUnidade.push_back(c_u);
                return true;
            ...
            // atualizar a quantidade do mesmo lote
            total = 0; // inicializar total
            for (unsigned i(0); i < CervejaUnidade.size(); i++)
            {
                if(CervejaUnidade[i].getCodigo() == c_u.getCodigo())
                {
                    total = c_u.getQntUnidade();
                    CervejaUnidade[i].setCalculaEstoque(total);
                    return true;
                }
            }
        }
